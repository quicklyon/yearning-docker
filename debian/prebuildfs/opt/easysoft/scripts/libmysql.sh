...
wait_for_mysql() {
    ...
    local mysql_host="${1:-$DB_HOST}"
    local mysql_port="${2:-$DB_PORT}"
...
#########################
mysql_init_db() {
    local init_db=${1:-DB_NAME}
    local -a args=("--host=$DB_HOST" "--port=$DB_PORT" "--user=$DB_USER" "--password=$DB_PASSWORD")
...
mysql_import_to_db() {
    ...
    local -a args=("--host=$DB_HOST" "--port=$DB_PORT" "--user=$DB_USER" "--password=$DB_PASSWORD" "$db_name")
    local command="/usr/bin/mysql"
